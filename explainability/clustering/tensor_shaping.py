...
def reshape_for_clustering_universal(
    embeddings: torch.Tensor | np.ndarray, channel_dim_index: int
    ...
    if isinstance(embeddings, np.ndarray):
        return embeddings.transpose(permute_tuple).reshape(
            -1, embeddings.shape[channel_dim_index]
        )
    elif isinstance(embeddings, torch.Tensor):
        return embeddings.permute(permute_tuple).reshape(
            -1, embeddings.shape[channel_dim_index]
        )
