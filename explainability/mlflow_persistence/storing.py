...
def ensure_mlflow_run(experiment_name: str, run_id: str | None = None):
    ...
    # otherwise fail
    try:
        username = git.Repo(search_parent_directories=True).git.config(
            "--get", "user.name"
        )
    except Exception:
        ...
        if username is None:
            raise ValueError(
                "Username not found in git config or MLFLOW_TRACKING_USERNAME environment variable."
            )
    ...
    if run_id is not None:
        ...
        run = client.create_run(
            ...
            tags={
                ...
                MLFLOW_GIT_REPO_URL: git.Repo(
                    search_parent_directories=True
                ).remotes.origin.url,
                MLFLOW_GIT_COMMIT: git.Repo(
                    search_parent_directories=True
                ).head.commit.hexsha,
...
def artifact_exists(
    client: MlflowClient, run_id: str, artifact_path: str, file_name: str
) -> bool:
...
def upload_image_if_missing(
    ...
    if artifact_exists(client, run_id, artifact_subdir, file_name):
        print(
            f"Skipping upload — '{file_name}' already in MLflow under {artifact_subdir}/"
        )
        return False
    else:
        ...
        client.log_artifact(
            run_id,
            local_path=local_image_path.as_posix(),
