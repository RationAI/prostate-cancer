...
class ImageBuilder(ABC):
    ...
    def __init__(self, vis_mode: VisualisationMode, save_dir: Path | str, filename: str) -> None:
    ...
    def _to_nwhc(data: NDArray) -> NDArray:
        """Converts data to [N, W, H, C] format, where:
            N - batch size
            W - width in pixels
            H - height in pixels
        ...
        if data.ndim == 4:  # tile is image
            ...
            return data.transpose((0, 2, 3, 1))  # [N, C, W, H] into sensible [N, W, H, C]
...
class ImageAssembler(ImageBuilder, ABC):
    ...
    def _resize_to_tile_size(self, data: NDArray) -> NDArray:
        interpolation_map = dict(nearest=0, bilinear=1, cubic=2)
        ...
        if self.tile_size != width or self.tile_size != height:
            ...
            data = scipy.ndimage.zoom(input=data, zoom=zoom, order=interpolation_map[self.interpolation])
...
class DiskMappedPatchAssembler(ImageAssembler):
    ...
    def __init__(
        ...
        super().__init__(image_size=image_size, tile_size=tile_size,
                         vis_mode=vis_mode, save_dir=save_dir, filename=filename,
        ...
        self.image = np.memmap(
            ...
            dtype=np.float32,
            mode='w+',
            shape=(self.image_size[1], self.image_size[0], self.image_size[2])  # row-first format (H, W, C)
        )
        self.count = np.memmap(
            save_path + '_count.nmp',
            dtype=np.int32,
            mode='w+',
            shape=(self.image_size[1], self.image_size[0], 1)  # row-first format (H, W, C)
        )

    def update(self, data: torch.Tensor, metadata: dict) -> None:
        logger.debug("Pasting tiles.")
        xs, ys = metadata['coord_x'] // self.scale_factor, metadata['coord_y'] // self.scale_factor
        data = self._preprocess_data(data)
        data = self._resize_to_tile_size(data)

        # Paste tiles onto mask
        for x, y, tile in zip(xs, ys, data):
            mm_h, mm_w, mm_c = self.image[y:y + self.tile_size, x:x + self.tile_size, :].shape  # noqa
            self.image[y:y + self.tile_size, x:x + self.tile_size, :] += tile[:mm_h, :mm_w, :mm_c]
...
class InMemoryHeatmapAssembler(ImageAssembler):
    ...
    def __init__(
        ...
        compressed_w = w // self.gcd_size_factor
        compressed_h = h // self.gcd_size_factor
        ...
        self.heatmap_accumulator = np.zeros(
            shape=(accum_h, accum_w, c),  # row-first format (H, W, C)
            dtype=np.float32
        )
    ...
    def update(self, data: torch.Tensor, metadata: dict) -> None:
        ...
        for xa, ya, xc, yc, tile in zip(xs_accum, ys_accum, xs_count, ys_count, data):
            mm_h, mm_w, mm_c = self.heatmap_accumulator[  # noqa
                    ya:ya + self.accumulator_tile_size,
                    xa:xa + self.accumulator_tile_size,
                    :].shape
            self.heatmap_accumulator[ya:ya + self.accumulator_tile_size,
                                     xa:xa + self.accumulator_tile_size, :] += tile[:mm_h, :mm_w, :mm_c]
            self.patch_overlap_counter[yc:yc + self.overlap_counter_tile_size,
                                       xc:xc + self.overlap_counter_tile_size, :] += 1
    ...
    def save(self) -> str:
        ...
        # Resize to full size
        if self.compress_accumulator_array:
            vips_im = vips_im.resize(self.gcd_size_factor, kernel=self.interpolation)
...
class JpegImageBuilder(ImageBuilder):
    ...
    def update(self, data: torch.Tensor, metadata: dict) -> None:
        data = self._preprocess_data(data)
        for img, filename in zip(data, metadata["filename"]):
