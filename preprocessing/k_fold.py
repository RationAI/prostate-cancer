...
def attach_group(
    base_slides_df: pd.DataFrame, annotations_source: pd.DataFrame, group_col: str
) -> pd.DataFrame:
    ...
    if base_slides_df[group_col].isna().any():
        raise ValueError(f"Missing '{group_col}' after joining annotations source.")

    return base_slides_df


def compute_folds(
    base_slides_df: pd.DataFrame, target_column: str, group_column: str, k: int
) -> pd.DataFrame:
    slides_with_folds = stratified_group_k_fold_split(
        base_slides_df, target_column, group_column, k
    )
...
def main(config: DictConfig, logger: MLFlowLogger) -> None:
    ...
    base_slides_df = attach_group(
        base_slides_df, annotations_source, config.group_column
    )
    fold_by_path = compute_folds(
        base_slides_df, config.target_column, config.group_column, config.k
    )

    if slides_df_512 is not None:
        apply_folds(
            slides_df_512, tiles_df_512, fold_by_path, f"{config.data.data_name}_512"
        )

    if slides_df_224 is not None:
        apply_folds(
            slides_df_224, tiles_df_224, fold_by_path, f"{config.data.data_name}_224"
        )
